...
Replace the character.repeat(i + 1) in your .push() call with a function call to padRow. Do not pass any arguments to it yet.
...
```js
assert.match(__helpers.removeJSComments(code), /push\(\s*?padRow\(\s*?\)\s*\)/);
assert.notMatch(__helpers.removeJSComments(code), /push\(\s*?padRow\(\s*?[^\s)]/);
